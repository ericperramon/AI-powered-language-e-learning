...
Cada unidad sigue la estructura: una lección de vídeo (`lesson_type = 'video'`, `sort_order` más bajo) seguida de lecciones de práctica por destreza (`lesson_type = 'exercise'`) cuya etiqueta visual (**Reading**, **Grammar**, **Speaking**) se detecta por coincidencia de texto en el `title` de la lección (no por posición ni por un campo dedicado) — ver `getSkillForLesson` en `app/dashboard/courses/[courseId]/page.tsx`. El vídeo de la unidad se renderiza inline dentro del acordeón de la unidad (no como fila bloqueada/desbloqueada); las lecciones de destreza sin coincidencia de título se muestran con la etiqueta genérica "Lesson N".
El reproductor de vídeo (`components/courses/lesson-video-player.tsx`, reutilizado tanto en la página de unidades como en la página de lección) ya no se reproduce inline ni tiene botón manual de "marcar como visto": al pulsar play se abre un overlay a pantalla completa con el vídeo embebido (YouTube o Vimeo). Al detectar el fin del vídeo vía la API postMessage del reproductor (YouTube: `onStateChange` con `info === 0`; Vimeo: evento `finish` del protocolo postMessage crudo — *no* `ended`, que es el nombre usado por el SDK oficial `player.js`), se llama a la Server Action `markVideoWatched` (variante de `completeLessonVideo` sin `redirect()`) y se muestra un diálogo: *"Do you want to resolve any doubts with your tutor or start the exercises?"* con dos botones — **Tutor IA** (navega a la página dedicada `/dashboard/assistant`) y **Exercises** (abre la siguiente lección desbloqueada en una pestaña nueva mediante `window.open`). Si la lección no tiene `video_url` cargado todavía, se muestra el placeholder "Coming soon" sin ninguna acción disponible — **no hay forma de completar manualmente una lección de vídeo sin vídeo real cargado** (limitación conocida, ver Pendientes).
Existe un primer asistente IA global en la esquina inferior derecha de la interfaz, implementado como una cabeza de robot animada (carcasa blanca, ojos en matriz de LEDs, boca que se anima según el volumen del audio reproducido). Permite iniciar conversaciones persistentes en modo texto o modo voz. En modo voz, el navegador grava el audio del alumno con `MediaRecorder` y lo envía como `multipart/form-data` a `/api/assistant`; en modo texto se envía JSON. La ruta server-side `/api/assistant` reenvía texto al webhook `N8N_ASSISTANT_TEXT_WEBHOOK_URL` o audio al webhook `N8N_ASSISTANT_AUDIO_WEBHOOK_URL`, y la respuesta (texto o audio binario) se reproduce/muestra en el cliente. No hay llamada directa a OpenAI Realtime desde la app: todo el procesamiento IA del asistente pasa por n8n. La ruta exige usuario autenticado y aplica un rate limit por usuario mediante el guard `lib/assistant/guard.ts`; sin sesión devuelve `401` y al superar el límite `429`. El refresco de sesión SSR de Supabase lo gestiona `proxy.ts` en la raíz (equivalente al middleware en Next.js 16).
Además del widget flotante, existe una página dedicada del asistente en `/dashboard/assistant` (acepta `?courseId=&lessonId=` para inyectar contexto de curso/lección), pensada como destino del botón "Tutor IA" del diálogo post-vídeo. El componente `AiAssistant` acepta una prop `variant: "floating" | "page"`; en modo `"page"` se renderiza expandido y sin el botón robot flotante. `components/ai-assistant-mount.tsx` oculta el widget flotante global cuando la ruta activa es `/dashboard/assistant`, para no duplicar el chat en pantalla.
...
9. En cada lección, el alumno completa primero el vídeo (en overlay a pantalla completa) y, tras el diálogo post-vídeo, responde los ejercicios de la siguiente lección de destreza (Reading/Grammar/Speaking) que se abre en una pestaña nueva.
...
- Las lecciones normales siguen el orden: vídeo (lección independiente, `lesson_type = 'video'`), lecciones de destreza con ejercicios interactivos corregidos en servidor (`lesson_type = 'exercise'`, etiquetadas Reading/Grammar/Speaking por título) y vuelta a la página de unidades al completar cada una.
...
- `app/dashboard/courses/[courseId]/actions.ts` (Server Actions de ejercicios, lecciones y exámenes; `readExerciseItemIds` soporta `pairs[]` y `sentences[]`; `markVideoWatched` es la variante de `completeLessonVideo` sin `redirect()` usada por el nuevo flujo de vídeo)
- `app/dashboard/courses/[courseId]/lessons/[lessonId]/page.tsx` (página de lección con todos los renderers de ejercicio)
- `app/dashboard/courses/[courseId]/page.tsx` (página de unidades; `getSkillForLesson` asigna icono/etiqueta Reading/Grammar/Speaking por título de lección)
- `components/courses/lesson-video-player.tsx` (overlay de vídeo a pantalla completa + detección de fin + diálogo Tutor IA/Exercises; reutilizado en course page y lesson page)
- `lib/video.ts` (`getVideoEmbedUrl`, `getPlayableEmbedUrl` — parseo de URLs de YouTube/Vimeo compartido)
- `app/dashboard/assistant/page.tsx` (página dedicada del Tutor IA, destino del botón del diálogo post-vídeo)
- `components/ai-assistant-mount.tsx` (oculta el widget flotante del asistente en `/dashboard/assistant`)
...
- **`completeLessonVideo` (en `actions.ts`) ha quedado sin uso** tras introducir `markVideoWatched` y eliminar el botón manual "Mark video as watched and continue". Pendiente decidir si se elimina o se mantiene como fallback.
- **Lecciones de vídeo sin `video_url` cargado no se pueden completar**: al quitar el botón manual, si una unidad aún no tiene vídeo real subido, el alumno queda bloqueado en esa lección (no hay alternativa para marcarla como vista). Pendiente decidir si se reintroduce algún mecanismo para este caso (p. ej. solo visible para `superadmin`/preview).
- **Los `lesson_type` de las lecciones de destreza deben ser `'exercise'`, no `'mixed'`**: si quedan en `'mixed'` (valor por defecto del esquema), la página de lección muestra también una pestaña de vídeo que no debería existir ahí. Verificar/corregir manualmente en Supabase al cargar contenido nuevo.
- Para que el reproductor de Vimeo detecte el fin del vídeo, el privacy setting de "¿Dónde se puede insertar este video?" debe permitir el dominio de la app (no solo el privacy setting general "Privado/Público"); restringir a dominios específicos normalmente requiere plan Vimeo Plus o superior.
